...
final class DirectoryContentViewController: UICollectionViewController {
    ...
    override func viewDidLayoutSubviews() {
        ...
        collectionViewLayout.footerReferenceSize = CGSize(width: view.bounds.width,
                                                          height: collectionView.frame.height - CGFloat(viewModel.numberOfItems(inSection: 0)) * collectionViewLayout.itemSize.height)
        if isFirstLayout {
            isFirstLayout = false
    ...
    func syncWithViewModel(_ animated: Bool) {
        ...
        let editBarButtonItem = viewModel.isEditActionHidden ? nil : UIBarButtonItem(title: viewModel.editActionTitle,
                                                                                     style: .plain,
                                                                                     target: self,
                                                                                     action: #selector(DirectoryContentViewController.handleEditButtonTap))
    ...
    @objc func handleDeleteButtonTap() {
        ...
        viewModel.deleteItems(at: viewModel.indexPathsOfSelectedCells) { [weak self] result in
            ...
            strongSelf.delegate?.directoryContentViewController(strongSelf,
                                                                didChangeEditingStatus: strongSelf.viewModel.isEditing)
        }
    }
